...
class BiEncoderTrainer(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        if model_file:
            ...
            pos_ids = torch.arange(512).reshape(1,-1)
            saved_state.model_dict['question_model.embeddings.position_ids'] = pos_ids
            saved_state.model_dict['ctx_model.embeddings.position_ids'] = pos_ids
            set_cfg_params_from_state(saved_state.encoder_params, cfg)

        tensorizer, model, optimizer = init_biencoder_components(cfg.encoder.encoder_model_type, cfg)
        # we initialize two independent tokenizers here to reduce memory cost
        tensorizer_q , tensorizer_ctx = tensorizer
        ...
        self.tensorizer_q = tensorizer_q
        self.tensorizer_ctx = tensorizer_ctx
        self.start_epoch = 0
        self.start_batch = 0
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            rep_positions = ds_cfg.selector.get_positions(biencoder_input.question_ids, self.tensorizer_q)
    ...
    def validate_average_rank(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            ds_cfg = self.ds_cfg.dev_datasets[dataset]
            encoder_type = ds_cfg.encoder_type
            rep_positions = ds_cfg.selector.get_positions(biencoder_input.question_ids, self.tensorizer_q)  # todo tensorizer_q

            # split contexts batch into sub batches since it is supposed to be too large to be processed in one batch
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):

                q_ids, q_segments = (
                    (biencoder_input.question_ids.to(cfg.device), biencoder_input.question_segments.to(cfg.device)) if j == 0 else (None, None)
                )
                ...
                q_attn_mask = self.tensorizer_q.get_attn_mask(q_ids)  # todo tensorizer_q
                ctx_attn_mask = self.tensorizer_ctx.get_attn_mask(ctx_ids_batch)
                with torch.no_grad():
                    q_dense, ctx_dense = self.biencoder(
                        ...
                        ctx_attn_mask,
                        # q_ids.to(cfg.device),
                        # q_segments.to(cfg.device),
                        # q_attn_mask.to(cfg.device),
                        # ctx_ids_batch.to(cfg.device),
                        # ctx_seg_batch.to(cfg.device),
                        # ctx_attn_mask.to(cfg.device),
                        encoder_type=encoder_type,
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            biencoder_batch = biencoder.create_biencoder_input(
                ...
                num_other_negatives,
                shuffle=True,
                shuffle_positives=shuffle_positives,  # shuffle_positive默认为false，mix模式则修改为true
            ...
            rep_positions = selector.get_positions(biencoder_batch.question_ids, self.tensorizer_q)  # todo tensorizer_q
            ...
            # if data_iteration % eval_step == 0:  # 按照默认参数，每个epoch eval一次，与下方eval重复
            #     logger.info(
            ...
            #     self.biencoder.train()
        # TODO report train acc
        total_samples = batches * cfg.train.batch_size * self.distributed_factor
        correct_ratio = float(epoch_correct_predictions / total_samples)

        logger.info("Epoch finished on %d", cfg.local_rank)
        epoch_loss = (epoch_loss / epoch_batches) if epoch_batches > 0 else 0
        logger.info("Train Av Loss per epoch=%f. correct prediction ratio  %d/%d ~  %f",
                    epoch_loss,
...
def _do_biencoder_fwd_pass(
    ...
    tensorizer_q: Tensorizer,
    tensorizer_ctx: Tensorizer,
    cfg,
    encoder_type: str,
    rep_positions=0,
    loss_scale: float = None,
    ...
    q_attn_mask = tensorizer_q.get_attn_mask(input.question_ids)  # todo tensorizer_q
    ctx_attn_mask = tensorizer_ctx.get_attn_mask(input.context_ids)
