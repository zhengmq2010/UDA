...
def main(cfg: DictConfig):
    ...
    tensorizer_q, tensorizer_ctx = tensorizer[0], tensorizer[1]
    ...
    data = gen_ctx_vectors(cfg, shard_passages, encoder, tensorizer_ctx, True)

    file = cfg.out_file + "_" + str(cfg.shard_id)
    pathlib.Path(os.path.dirname(file)).mkdir(parents=True, exist_ok=True)  # 确保parent文件夹存在
