...
class FiDT5(transformers.T5ForConditionalGeneration):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        if input_ids != None:
            ...
            if input_ids.dim() == 3:  # (batch, n_ctx, len_ctx)
                self.encoder.n_passages = input_ids.size(1)
            input_ids = input_ids.view(input_ids.size(0), -1)
        if attention_mask != None:
            attention_mask = attention_mask.view(attention_mask.size(0), -1)
        return super().forward(  # 跳到EncoderWrapper.forward
            input_ids=input_ids,
    ...
    def generate(self, input_ids, attention_mask, max_length):
        self.encoder.n_passages = input_ids.size(1)
        return super().generate(  # 会跳到cross_attention_forward计算score
            input_ids=input_ids.view(input_ids.size(0), -1),
            attention_mask=attention_mask.view(attention_mask.size(0), -1),
    ...
    def get_crossattention_scores(self, context_mask):
        ...
        for mod in self.decoder.block:  # self.decoder来自T5ForConditionalGeneration的源码
            scores.append(mod.layer[1].EncDecAttention.score_storage)  # block里一共12层
        scores = torch.cat(scores, dim=2)
        bsz, n_heads, n_layers, _ = scores.size()  # n_head=12, n_layer=12 for t5-base
        # batch_size, n_head, n_layers, n_passages, text_maxlength
        scores = scores.view(bsz, n_heads, n_layers, n_passages, -1)
        scores = scores.masked_fill(~context_mask[:, None, None], 0.)
        scores = scores.sum(dim=[1, 2, 4])  # n_head, n_layers, text_len
        ntokens = context_mask.sum(dim=[2]) * n_layers * n_heads
        scores = scores/ntokens  # avg aggregation
        return scores
    ...
    def overwrite_forward_crossattention(self):
        ...
        """
        for mod in self.decoder.block:
            attn = mod.layer[1].EncDecAttention  # 先创建实例attn
            attn.forward = types.MethodType(cross_attention_forward, attn)  # MethodType:把函数作为方法加入实例
...
class EncoderWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        attention_mask = attention_mask.view(bsz*self.n_passages, passage_length)  # -10000 is padding
        outputs = self.encoder(input_ids, attention_mask, **kwargs)  # jump to CheckpointWrapper.forward, output(n_ctx,len_ctx,768)
        outputs = (outputs[0].view(bsz, self.n_passages*passage_length, -1), ) + outputs[1:]  # (B*n_ctx,len_ctx,768)        return outputs
        return outputs
...
def cross_attention_forward(
    ...
    """
    assert(kv != None)  # kv:batch,n_txt*len_txt,768
    ...
    scores = torch.einsum("bnqd,bnkd->bnqk", q, k)  # 矩阵乘法
