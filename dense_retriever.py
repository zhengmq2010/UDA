...
def generate_question_vectors(
    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            # max_vector_len = max(q_t.size(0) for q_t in batch_tensors)  # 这里应该是要删掉
            # min_vector_len = min(q_t.size(0) for q_t in batch_tensors)  # 这里都是一样长，下面的也没必要padding
...
def validate(
    ...
    mrr_10 = []
    for rank in match_stats.questions_doc_hits:
        hit_idx = [i for i in range(len(rank)) if rank[i] == True]
        if len(hit_idx) == 0:  # top10没命中
            mrr_10.append(0.0)
        else:
            if hit_idx[0] < 10:
                mrr_10.append(1 / (hit_idx[0] + 1))
            else:
                mrr_10.append(0.0)
    logger.info('Validation results: top@10 documents MRR %s', sum(mrr_10) / len(mrr_10))
...
def main(cfg: DictConfig):
    ...
    tensorizer = tensorizer[0]  # 定义了两个tokenizer
