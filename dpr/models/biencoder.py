    ...
    def create_biencoder_input(
        ...
        for sample in samples:
            ...
            if shuffle and shuffle_positives:
                ...
                if mode == 'train':  # 针对uGAR的3source训练集
                    positive_ctx = sample.positive_passages[0]  # 只取一个正样本 修改这里来改变增强类型 0ans,1tit,2sen,mix修改shuffle_positive参数
                else:
            ...
            sample_ctxs_tensors = [  # TODO 改为tensorizer_ctx
                tensorizer_ctx.text_to_tensor(ctx.text, title=ctx.title if (insert_title and ctx.title) else None)
                for ctx in all_ctxs  # 一正+n负
            ]
            ...
            if query_token:
                ...
                if query_token == "[START_ENT]":
                    query_span = _select_span_with_token(question, tensorizer_q, token_str=query_token)  # tensorizer_q
                    question_tensors.append(query_span)
                else:
                    question_tensors.append(tensorizer_q.text_to_tensor(" ".join([query_token, question])))
            else:
                question_tensors.append(tensorizer_q.text_to_tensor(question))
